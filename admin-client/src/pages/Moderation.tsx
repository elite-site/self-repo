...
const StatusBadge = ({ status }: { status: string }) => {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    CHANGES_REQUESTED: { label: 'Changes Requested', cls: 'bg-orange-50 text-orange-700 border-orange-200' },
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-[#0B192C]">Moderation Queue</h1>
          <p className="text-xs text-neutral-500">Review and decide on student submissions</p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-neutral-100 rounded-xl p-1 w-fit border border-neutral-200">
        {tabs.map((t) => {
          const Icon = t.icon;
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                isActive ? 'bg-white text-[#DC2626] shadow-sm' : 'text-neutral-500 hover:text-neutral-700'
              }`}
            >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl border border-red-200 gap-3">
        ...
        <div className="flex flex-col items-center justify-center h-64 bg-white rounded-2xl border border-[#E2E8F0] gap-3">
          ...
          <p className="text-sm font-semibold text-neutral-500">Queue is empty</p>
          <p className="text-xs text-neutral-400">No pending {activeTab} to review.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden">
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-[#E2E8F0] bg-[#F8FAFC]">
                <button
                  ...
                  disabled={idx === 0}
                  className="flex items-center gap-1 text-xs font-semibold text-neutral-500 disabled:opacity-30 hover:text-[#0B192C] cursor-pointer"
                >
                ...
                {currentItem && (
                  <>
                    ...
                    <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#E2E8F0]">
                      ...
                      <div>
                        <div className="font-bold text-[#0B192C] text-sm">{currentItem.studentName}</div>
                    ...
                    {currentItem.title && (
                      <p className="text-sm font-semibold text-[#0B192C] mb-3">{currentItem.title}</p>
                    )}
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white rounded-2xl border border-[#E2E8F0] p-5 space-y-4">
              <h3 className="text-sm font-extrabold text-[#0B192C] uppercase tracking-wide">Decision</h3>
              ...
              {(action === 'reject' || action === 'changes') && (
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full text-xs border border-[#E2E8F0] rounded-lg p-3 resize-none focus:outline-none focus:border-[#DC2626] focus:ring-1 focus:ring-[#DC2626] text-[#0B192C]"
                  />
                  {!reason.trim() && (
              ...
              {action && (
                <button
                  ...
                  className="w-full py-3 rounded-lg text-xs font-bold bg-[#0B192C] text-white hover:bg-[#0B192C]/90 disabled:opacity-50 transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
